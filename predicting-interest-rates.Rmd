---
title: "Predicting Inflation Expectation"
author: "Noah Love and Blinda Tian"
date: "4/18/2021"
output: html_document
---

[The Github Repo](https://github.com/noahlove/data-mining-final "The Github Repo")
...
consumer_price_index_urban <- fredr(series_id = "CPIAUCSL") %>%
  rename("consumer_price_index_urban" = value) %>%
  select(-realtime_start, -realtime_end, -series_id) %>%
  mutate(dates = as.numeric(date))


inflation_expectation <- fredr(series_id = "MICH") %>%
  rename("inflation_expectation" = value) %>%
  select(-realtime_start, -realtime_end, -series_id) %>%
  mutate(dates = as.numeric(date))

df <-
  merge(consumer_price_index_urban, inflation_expectation, by = "dates") %>%
  select(-date.x, -date.y)

ten_year_treasury <- fredr(series_id = "DGS10") %>%
...
ten_minus_two_treasury <- fredr(series_id = "T10Y2Y") %>%
  rename("ten_minus_two_treasury" = value) %>%
  select(-realtime_start, -realtime_end, -series_id) %>%
  mutate(dates = as.numeric(date))

df <-
  merge(df, ten_minus_two_treasury, by = "dates", all = TRUE) %>%
  select(-date)

ten_minus_three_months_treasury <- fredr(series_id = "T10Y3M") %>%
...
real_gdp <- fredr(series_id = "GDPC1") %>%
...
eff_fed_funds_rate <- fredr(series_id = "FEDFUNDS") %>%
...
thirty_year_fixed_mortgage <- fredr(series_id = "MORTGAGE30US") %>%
  rename("thirty_year_fixed_mortgage" = value) %>%
  select(-realtime_start, -realtime_end, -series_id) %>%
  mutate(dates = as.numeric(date))

df <-
  merge(df, thirty_year_fixed_mortgage, by = "dates", all = TRUE) %>%
  select(-date)


velocity_of_mtwo <- fredr(series_id = "M2V") %>%
...
all_employees_minus_farmers <- fredr(series_id = "PAYEMS") %>%
...
df <-
  merge(df,
        all_employees_minus_farmers,
        by = "dates",
        all = TRUE) %>%
  select(-date)

personal_savings_rate <- fredr(series_id = "PSAVERT") %>%
...
aaa_corporate_bond_yield <-  fredr(series_id = "AAA") %>%
  rename("aaa_corporate_bond_yield" = value) %>%
  select(-realtime_start, -realtime_end, -series_id) %>%
  mutate(dates = as.numeric(date))

df <-
  merge(df, aaa_corporate_bond_yield, by = "dates", all = TRUE) %>%
  select(-date)


personal_consumption_expenditures <- fredr(series_id = "PCE") %>%
...
industrial_production_index <- fredr(series_id = "INDPRO") %>%
...
federal_debt_total <- fredr(series_id = "GFDEBTN") %>%
...
labor_force_participation_rate <- fredr(series_id = "CIVPART") %>%
...
consumer_price_index_nationwide <-
  fredr(series_id = "CPALTT01USM657N") %>%
...
s_and_p <- fredr(series_id = "CSUSHPINSA") %>%
...
m_three <- fredr(series_id = "MABMM301USM189S") %>%
...
federal_surplus_or_deficit <-  fredr(series_id = "FYFSD") %>%
...
df <-
  merge(df, federal_surplus_or_deficit, by = "dates", all = TRUE) %>%
  select(-date)



house_price_index_nationwide <- fredr(series_id = "USSTHPI") %>%
...
total_vehicle_sales <- fredr(series_id = "TOTALSA") %>%
...
federal_debt_percent_of_gdp <-  fredr(series_id = "GFDEGDQ188S") %>%
...
gz <-
  ggplot(correlation_df_melt, mapping = aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
...
index = sample(1:nrow(data_monthly_dateless),
               0.7 * nrow(data_monthly_dateless))
...
eval_metrics = function(model, df, predictions, target) {
  ...
  print(c("RMSE", as.character(round(sqrt(
    sum(resids2) / N
  ), 2)))) #RMSE
}
...
#km_out$cluster
plot(data_monthly$month,km_out$cluster)
